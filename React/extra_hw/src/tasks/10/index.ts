...
export function promisify<T>(argument: (callback: (data: ApiResponse<T>) => void) => void): () => Promise<T> {
  return () =>
...
function logPerson(person: Person) {
  return `- ${person.name}, ${person.age}, ${person.type === 'admin' ? person.role : person.occupation
    }`;
}
